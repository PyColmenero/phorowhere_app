...
var sharing_options = {
	message: 'Mira que sitio he encontrado en PhotoWhere! \n',
...
class Place {
	constructor() {
		...
		this.share.click(function () {
			...
			var url = "www.acolmenero.site/map?id=" + place.id;
			sharing_options.url = url;

			try {
				window.plugins.socialsharing.shareWithOptions(sharing_options);
			} catch (error) {
				$("h1").text(error);
			}
		})
	...
	map_marker_click(locations, id, isMarker, index) {
		...
		this.placedata.attr("class", "show-data");
		this.injectdata.css("display", "none");
